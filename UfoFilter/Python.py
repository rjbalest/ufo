...
class Context ( Filter.Context ):
	...
	function @RETURN_BY_REFERENCE@parent() {
		...
		$oid = $this->pid;
		$parent @ASSIGN_BY_REF@ $ufoSession->lookupContainer(%s, $suboid);
		return $parent;
	}
			""" % ( parent )
		return c
		...
		for v in self.vars:
			...
			if v.type == 'object':
				if v.is_reference:
					...
					c = c + "$this->%s->chown($uid, $gid, $recurse);\n\t\t\t" % ( v.name )
					nvars = nvars + 1
			elif v.type == 'collection':
	...
	def DELETEIMPL(self):
		...
		if self.atts.is_interface:
			...
			c = c + "if ($delegate) { $delegate->delete($recurse); }\n\t\t"

		if self.atts.extends:
			# Delete our parent
			c = c + "$parent = $this->parent();\n\t\t"
			c = c + "if ($parent) { $parent->delete($recurse); }\n\t\t"
	...
	def INITVIEWIMPL(self):
		...
		c = c + '\n<form %s action=\\"main.php?obj=%s&method=submit&page=$page\\" method=\\"post\\">' % (encoding, self.className)
		c = c + '\n<table class=\\"stats\\" border cellpadding=\\"0\\" style=\\"border-collapse: collapse\\" width=\\"50%\\">'
	...
	def SELECTOR(self):
		...
		for f in self.facets:
			if lfirst:
				c = c + "@PRIVATE@ $selected = array(\n\t"
				lfirst = False
			else:
				c = c + ","
			c = c + "'%s' => False\n\t" % f
		if lfirst:
			c = "@PRIVATE@ $selected = False;"
		else:
			c = c + ");"
		return c
	...
	def VIEWIMPL(self):
		...
		c = c + '\n<form %s action=\\"main.php?obj=%s&method=submit&page=%s\\" method=\\"post\\">' % (encoding, self.className, page)
		c = c + '\n<table class=\\"stats\\" border cellpadding=\\"0\\" style=\\"border-collapse: collapse\\" width=\\"50%\\">'
	...
	def CREATEPARENTCLASS(self):
		...
		if self.atts.extends:
			parent = self.atts.extends
			c = c + """
			$parent = $ufoSession->lookupContainer('%s',0);
			$parent->uid = $this->uid;
			$parent->gid = $this->gid;
			$parent->cid = $this->cid;
...
class @PREFIX@_@CLASSNAME@_base @PARENT_CLASS_DECL@ {
	...
	@PRIVATE@ $className;
	@PRIVATE@ $version = "@MAJORVERSION@.@MINORVERSION@";
	...
	function @PREFIX@_@CLASSNAME@_base( $argv=-1 ) {
		...
		$this->link_submit = "main.php?obj=@CLASSNAME@&method=submit&page=\$page";
	...
	function getSubmitLink() {
		global $page;
		return "main.php?obj=@CLASSNAME@&ufo[oid]={$this->oid}&method=submit&page=@CONTAINERNAME@";
	}
	// not generated currently
	function getDeleteLink() {
		global $page;
		return "main.php?obj=@CLASSNAME@&ufo[oid]={$this->oid}&method=delete&page={$page}";
	}
	// not generated currently
	...
	function link() {
		global $@GLOBALOBJECT@;
		$c = "<a href=\"http://{$@GLOBALOBJECT@->domain}/main.php?page=@CLASSNAME@\">@CLASSNAME@</a>";
		return $c;
	}
...
class @PREFIX@_@CONTAINERNAME@_base {
	...
	function initialize($argv=0) {
		...
		$ufoLog->info("@CLASSNAME@::initialize()");
	...
	function selectByGroups($id=Null) {
		global $ufoLog;
		$ufoLog->info("@CLASSNAME@::selectByGroup()");
	}
	...
	function @RETURN_BY_REFERENCE@add($obj=Null) {
		global $ufoLog;
		...
		if ($obj != Null) {
		   if ( is_array($obj) ) {
			...
			$o = new @PREFIX@_@CLASSNAME@(0);
		
			// initialize to defaults
			$o->initialize();
			
			$ufoLog->info("Setting uid to {$this->uid}");
			$ufoLog->info("Setting gid to {$this->gid}");
			$ufoLog->info("Setting cid to {$this->cid}");
			$o->uid = $this->uid;
			$o->gid = $this->gid;
			$o->cid = $this->cid;
	...
	function view_@CLASSNAME@($encoding='html') {
		...
		<form action=\"main.php?obj=@CONTAINERNAME@&method=control&page=@CONTAINERNAME@\" method=\"post\">
		";
		$lfirst = True;
		$idx = 0;
