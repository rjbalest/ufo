...
class UfoIndex:
    def __init__(self, atts):
        ...
        self.type = "index"
        self.name = ''
        # This is so it matches what UfoVar has
        self.index = True
        self.columns = []

        self.unique_values = {}
        self.unique_values['true'] = True
        self.unique_values['True'] = True
        self.unique_values['none'] = False
        self.unique_values[None] = False
        self.unique_values['false'] = False
        self.unique_values['False'] = False
    ...
    def initAttrs(self, atts):
        ...
        if atts.has_key('unique'):
            unique = atts['unique']
            if self.unique_values.has_key(unique):
                self.unique = self.unique_values[unique]
            else:
                print "Unique string %s invalid" % unique
                sys.exit(1)
        else:
            self.unique = False

        if atts.has_key('columns'):
            self.columns = atts['columns'].split(',')
...
class _UfoVar:
    def __init__(self, atts):
        ...
        # Possible values for unique/index attr
        self.index_values = {}
        self.index_values['true'] = True
        self.index_values['True'] = True
        self.index_values['none'] = False
        self.index_values[None] = False
        self.index_values['false'] = False
        self.index_values['False'] = False
    ...
    def initAttrs(self, atts):
        ...
        if atts.has_key('unique'):
            unq = atts['unique']
            if self.index_values.has_key(unq):
                self.unique = self.index_values[unq]
            else:
                print "Unique string %s invalid" % unq
                sys.exit(1)
            self.index = False
        elif atts.has_key('index'):
            idx = atts['index']
            if self.index_values.has_key(idx):
                self.index = self.index_values[idx]
            else:
                print "Index string %s invalide" % idx
                sys.exit(1)
            self.unique = False
        else:
            self.unique = False
    ...
    def viewRO_td(self):
        ...
        if self.type == "object":
            ...
            if self.is_reference:
                ...
                c = c + '\n<td class=\\"labelValue\\">" . @FCHAIN($this,get%s()->identifiableName())@ ."</td>' % (Balest.UCCNAME(self.name))
                return c

            elif self.select == 'multiple':
                 c = c + '\n<td class=\\"labelValue\\"><select>" . $this->get@UCCNAME(%s)@()->viewAsMenuOptions() ."</select></td>' % (self.name)
                 c = c + '\n<td class=\\"labelValue\\"><select>" . @FCHAIN($this,get%s()->viewAsMenuOptions())@ ."</select></td>' % (Balest.UCCNAME(self.name))
...
class _DojoVar( _UfoVar ):
    def formInput_td(self):
        ...
        if self.type == "bool":
            #c = c + '\n<td class=\\"labelValue\\"><input type=\\"checkbox\\" name=\\"ufo[%s]\\" value=\\"1\\" {$CHECKED[$this->%s]}></td>' % (self.name, self.name)
	    c = c + '\n<td class=\\"labelValue\\"><input type=\\"checkbox\\" name=\\"ufo[%s]\\" value=\\"1\\" {$CHECKED[$this->%s]} dojoType=\\"dijit.form.CheckBox\\"></td>' % (self.name, self.name)

            return c

        if self.type == "string":
            c = c + '\n<td class=\\"labelValue\\"><textarea dojoType=\\"dijit.form.Textarea\\" name=\\"ufo[%s]\\">{$this->%s} add notes here </textarea></td>' % (self.name, self.name)
            return c

        if self.type == "int":
	    constraint = self.constraintAttribute()
            c = c + '\n<td class=\\"labelValue\\"><input dojoType=\\"dijit.form.NumberSpinner\\" %s name=\\"ufo[%s]\\" value=\\"$this->%s\\"></td>' % ( constraint, self.name, self.name )
            return c
    ...
    # Attempt to build a dojo string constraint given a ufo constraint.
    def constraintAttribute( self ):
	c = ''
	if self.type == "int":
	    if self.constraint:
		if self.constraint.source == 'range':
		    c = 'constraints=\\"{min:%s,max:%s,places:0}\\" smallDelta=\\"%s\\"' % (self.constraint.min, self.constraint.max, self.constraint.incr)
	return c

# Alias UfoVar to _UfoVar
UfoVar = _DojoVar
...
\t */\n\t""" % (self.doc, t)
    ...
    def viewRO(self):
        c = ""
        c = c + '\n<tr>'
        c = c + '\n<td class=\\"label\\">" . @FCHAIN($this,get%s()->link())@ . "</td>' % Balest.UCCNAME(self.name)
        c = c + self.viewRO_td()
        c = c + '\n</tr>'
        return c
...
class UfoGenerator(handler.ContentHandler):
    ...
    def start_var(self, attrs):
        ...
        if self.in_collection:
            ...
            self.currentVar = var
            self.appendVar(var)
            if var.index or var.unique:
                self.appendIndex(UfoIndex(dict(attrs, columns=attrs['name'])))
        if var.namespace:
    ...
    def start_index(self, attrs):
        self.currentIndex = UfoIndex(attrs)
        self.appendIndex(self.currentIndex)
        self.in_index = True
